...
export function createDocumentService(http: HttpClient, streamSse: SseStreamFn) {
   ...
   */
  const invalidateDocumentListCache = (): void => {
  ...
  return {
    ...
    async getAllDocuments(page = 1, pageSize = 3, courseId?: string): Promise<PagedDocuments> {
      ...
      const request = http
        .get<{ data: { items: BackendDocument[]; totalCount: number; page: number; pageSize: number; totalPages: number } }>(url)
        .then(response => {
    ...
    async getDocument(courseId: string, documentId: string): Promise<Document> {
      const response = await http.get<{ data: BackendDocument }>(`/api/courses/${courseId}/documents/${documentId}`);
    ...
    async getDownloadUrl(courseId: string, documentId: string): Promise<string> {
      const response = await http.get<{ data: string }>(`/api/courses/${courseId}/documents/${documentId}/download-url`);
    ...
    async deleteDocument(courseId: string, documentId: string): Promise<void> {
      await http.delete(`/api/courses/${courseId}/documents/${documentId}`);
      invalidateDocumentListCache();
    },
    ...
    async deleteChatHistory(courseId: string, documentId: string): Promise<void> {
      await http.delete(`/api/courses/${courseId}/documents/${documentId}/chat`);
    },
    ...
    async deleteNote(courseId: string, documentId: string, noteId: string): Promise<void> {
      await http.delete(`/api/courses/${courseId}/documents/${documentId}/notes/${noteId}`);
    },
...
export function createQuizSubmissionService(http: HttpClient) {
  ...
  return {
    async getAllSubmissions(page = 1, pageSize = 20): Promise<PagedQuizSubmissions> {
      ...
      const request = http
        .get<{ data: { items: unknown[]; totalCount: number; page: number; pageSize: number; totalPages: number } }>(url)
        .then(response => {
          const d = response.data.data;
          const value = {
            items: d.items.map(mapQuizSubmission),
    ...
    async getCoverage(): Promise<QuizSubmissionCoverage> {
      ...
      const request = http
        .get<{ data: Partial<QuizSubmissionCoverage> }>(url)
        .then(response => {
          const d = response.data.data;
          return {
    ...
    async getPendingMaterials(): Promise<PendingMaterial[]> {
      ...
      const request = http
        .get<{ data: PendingMaterial[] | null }>(url)
