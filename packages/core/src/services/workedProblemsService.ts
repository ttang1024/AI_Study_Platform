...
export function createWorkedProblemsService(http: HttpClient) {
  return {
    ...
    async getProblems(documentId: string): Promise<WorkedProblem[]> {
      const res = await http.get<{ data: WorkedProblem[] }>(`/api/documents/${documentId}/worked-problems`);
      return res.data.data ?? [];
    },

    async generateProblems(documentId: string, difficulty: string, count: number): Promise<WorkedProblem[]> {
      const res = await http.post<{ data: WorkedProblem[] }>(
    ...
    async submitAttempt(problemId: string, userAnswer: string): Promise<ProblemAttempt> {
      const res = await http.post<{ data: ProblemAttempt }>(
    ...
    async getMastered(): Promise<Set<string>> {
      const res = await http.get<{ data: string[] }>('/api/worked-problems/mastered');
      return new Set(res.data.data ?? []);
    },

    async toggleMastered(problemId: string): Promise<boolean> {
      const res = await http.post<{ data: boolean }>(`/api/worked-problems/mastered/${problemId}`);
      return res.data.data;
    },
  };
}
