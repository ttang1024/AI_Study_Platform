...
export function createPracticeService(http: HttpClient) {
  return {
    async generate(opts: GenerateOptions = {}): Promise<PracticeTest> {
      ...
      const res = await http.get<{ data: PracticeTest }>(`/api/practice/generate?${params.toString()}`);
    ...
    async generateSmartSession(): Promise<PracticeTest> {
      const res = await http.get<{ data: PracticeTest }>('/api/practice/smart-session');
      return res.data.data;
    },

    async submit(results: PracticeResultItem[]): Promise<PracticeTestSummary> {
      const res = await http.post<{ data: PracticeTestSummary }>('/api/practice/submit', { results });
      return res.data.data;
    },
  };
}
