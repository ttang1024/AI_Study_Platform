...
// ── FontSize extension ───────────────────────────────────────────────────────
// TipTap ships no font-size mark; this stores one as an inline style on textStyle.
...
/** The extension set every rich-text surface in the app writes with, so their HTML stays interchangeable. */
export const EDITOR_EXTENSIONS = [StarterKit, TextStyle, Color, FontSize];
...
/** Closes the popover when the next mousedown lands outside `ref`. */
function useDismissOnOutsideClick(ref: React.RefObject<HTMLElement | null>, close: () => void) {
  useEffect(() => {
...
const Divider = () => <div className="w-px h-4 bg-zinc-200 mx-1" />;

/**
 * The formatting row shared by every rich-text surface. `children` is slotted at the end of the
 * row — the note editor puts its save indicator there.
 */
export function EditorToolbar({ editor, className, children }: {
  editor: any; className?: string; children?: React.ReactNode;
}) {
  return (
    <div className={cn(
      'flex items-center gap-0.5 border-b border-[var(--border-color)] px-3 py-1.5 bg-[var(--bg-sidebar)] flex-wrap',
      className,
    )}>
