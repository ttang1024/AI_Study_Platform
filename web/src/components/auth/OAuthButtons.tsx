import { buildOAuthUrl, GOOGLE_CLIENT_ID, GITHUB_CLIENT_ID } from '../../utils/oauth';

const buttonClass = 'flex w-full items-center justify-center gap-3 rounded-xl border border-zinc-200 bg-white px-4 py-3 text-sm font-medium text-zinc-700 shadow-sm transition hover:bg-zinc-50 hover:border-zinc-300 disabled:cursor-not-allowed disabled:opacity-40';

/**
 * The Google and GitHub sign-in buttons plus the "or" divider under them.
 *
 * <p>A provider whose client id is not configured stays visible but disabled, so the page looks the
 * same in every environment and a missing key is obvious rather than silently hiding a login route.</p>
 *
 * @param verb "Continue" on sign-in, "Sign up" on registration.
 */
