...
public interface IFlashcardRepository : IRepository<Flashcard>
{
    ...
    /// <summary>
    /// Cards of one difficulty, counted in the database. <paramref name="difficulty"/> must be lowercase;
    /// the comparison is case-insensitive on the stored value.
    /// </summary>
    Task<int> CountByDifficultyAsync(Guid userId, string difficulty, CancellationToken cancellationToken = default);
}
